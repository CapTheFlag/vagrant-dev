...
#
# Script to start and stop profiling
#
    ...
    if [ -d "/usr/lib/php5/${XDEBUG_VERSION}" ];then
        ...
        case "$1" in
            ...
            on|ON)
                ...
                sed "s@^xdebug.profiler_enable=0@xdebug.profiler_enable=1@" ${CONFIG_FILE}.bkp > ${CONFIG_FILE}
                ;;

            off|OFF)
                cp -f ${CONFIG_FILE} ${CONFIG_FILE}.bkp
                sed "s@^xdebug.profiler_enable=1@xdebug.profiler_enable=0@" ${CONFIG_FILE}.bkp > ${CONFIG_FILE}
                ;;

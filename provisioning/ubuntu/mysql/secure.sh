...
# 20090928: kbp: modified to allow non-interactive use
...
if [ $# -gt 0 ]; then
    if [ $# -ne 2 ]; then
        echo "Non-interactive usage: $0 old_root_password new_root_password"
        exit 1
    else
        noninteractive=yes
        password=$1
        password1=$2
        if [ -z $password1 ]; then
            echo "Non-interactive usage: new_root_password cannot be blank"
            exit 1
        fi
    fi
fi
...
echo "Could not authenticate to mysql, wrong password ?"
exit 1
fi
}

ni_set_root_password() {
...
remove_anonymous_users
echo $echo_n "Disallowing remote root login..."
remove_remote_root
echo $echo_n "Removing test databse..."
remove_test_database
echo $echo_n "Reloading privilege tables..."
reload_privilege_tables
cleanup
exit
fi
